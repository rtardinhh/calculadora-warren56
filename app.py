...
# Extra CSS para ocultar la cabecera/toolbar y asegurar que el fondo cubra toda la pantalla
extra_css = """
<style>
/* Oculta la cabecera/toolbar de Streamlit */


/* Quita márgenes que puedan dejar una franja blanca */
html, body, .stApp {
  height: 100%;
  margin: 0;
  padding: 0;
...
/* Asegura que el background definido en .stApp cubra toda la pantalla */
.stApp {
  min-height: 100vh;
...
/* Limita tamaño de logos para que no rompan el layout */
...
def validate_inputs(inp: Inputs) -> List[str]:
    ...
    # Verificar las tres areas por tipo
    if inp.area_top_cm2 <= 0:
        errors.append("El área del cordón superior debe ser positiva.")
    if inp.area_bot_cm2 <= 0:
        errors.append("El área del cordón inferior debe ser positiva.")
    if inp.area_diag_cm2 <= 0:
        errors.append("El área de las diagonales debe ser positiva.")
...
def warren_geometry(L: float, H: float, panels: int) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]]]:
    d = L / panels
    # Nodos inferiores: n+1
    bot_nodes = [(i * d, 0.0) for i in range(panels + 1)]
    # Nodos superiores: n (desplazados medio panel para centrar y evitar voladizos)
    top_nodes = [((i + 0.5) * d, H) for i in range(panels)]
...
def member_segments(L: float, H: float, panels: int) -> Dict[str, Tuple[Tuple[float, float], Tuple[float, float]]]:
    ...
    # Cuerda Inferior (CI): n miembros
    for i in range(panels):
        seg[f"CI{i+1}"] = (bot[i], bot[i + 1])

    # Cuerda Superior (CS): n-1 miembros (Conectando solo los nodos superiores internos)
    for i in range(panels - 1):
        seg[f"CS{i+1}"] = (top[i], top[i + 1])

    # Diagonales (D): 2n miembros en zig-zag (bot-top-bot)
    for i in range(panels):
        seg[f"D{2*i+1}"] = (bot[i], top[i])
        seg[f"D{2*i+2}"] = (top[i], bot[i+1])
...
def plot_truss(res: Dict[str, Any], member_evals: List[Dict[str, Any]]) -> go.Figure:
    ...
    # Supports (Dibujados primero para que queden atrás)
    x0, y0 = bot_nodes[0]
    xr, yr = bot_nodes[-1]

    fig.add_trace(go.Scatter(
        x=[x0], y=[y0], mode="markers",
        marker=dict(symbol="triangle-up", size=30, color="#94A3B8"),
    ...
    # Nodes (Dibujados al final para tapar las uniones)
    fig.add_trace(go.Scatter(
        x=[p[0] for p in bot_nodes], y=[p[1] for p in bot_nodes],
        mode="markers+text",
        marker=dict(size=14, color="#60A5FA", line=dict(width=2, color="#0B1E33")),
        text=[str(i) for i in range(len(bot_nodes))],
        textposition="bottom center", textfont=dict(size=11, color="#EAF2FA"),
        showlegend=False
    ))
    fig.add_trace(go.Scatter(
        x=[p[0] for p in top_nodes], y=[p[1] for p in top_nodes],
        mode="markers",
        marker=dict(size=14, color="#F97316", line=dict(width=2, color="#0B1E33")),
        showlegend=False
    ))
    ...
    fig.update_layout(
        ...
        xaxis=dict(visible=False, range=[-0.1*L, 1.1*L]),
        yaxis=dict(visible=False, scaleanchor="x", scaleratio=1, range=[-0.5*H, 1.5*H]),
    )
...
def page_calc() -> None:
    ...
    area_top_cm2 = st.sidebar.number_input("Área cordón superior (cm²)", min_value=0.01, value=0.8, step=0.1, format="%.3f")
    area_bot_cm2 = st.sidebar.number_input("Área cordón inferior (cm²)", min_value=0.01, value=0.8, step=0.1, format="%.3f")
    area_diag_cm2 = st.sidebar.number_input("Área diagonales (cm²)", min_value=0.01, value=0.4, step=0.1, format="%.3f")
    ...
    inp = Inputs(L=L, H=H, panels=panels, P_total=P_total, area_top_cm2=area_top_cm2, area_bot_cm2=area_bot_cm2, area_diag_cm2=area_diag_cm2, material=material)
    ...
    if run:
        ...
        if errs:
            ...
            min_area = min(inp.area_top_cm2, inp.area_bot_cm2, inp.area_diag_cm2)
            safety = truss.evaluate_safety(material=inp.material, section_area_cm2=min_area)
            ...
            st.session_state.last_run = {
                ...
                "inputs": {
                    ...
                    "panels": inp.panels,
                    "P_total": inp.P_total,
                    "area_top_cm2": inp.area_top_cm2, "area_bot_cm2": inp.area_bot_cm2, "area_diag_cm2": inp.area_diag_cm2,
    ...
    with tabs[0]:
        ...
        # --- NUEVO: Cuadro de Validación Estática para el profesor ---
        st.markdown("---")
        st.subheader("📊 Validación Estructural (Estática)")
        n_p = int(res["panels"])
        j_n = 2 * n_p + 1  # n_p+1 abajo + n_p arriba
        m_e = 4 * n_p - 1  # n_p abajo + (n_p-1) arriba + 2*n_p diagonales
        r_a = 3            # Reacciones estándar

        c_v1, c_v2, c_v3 = st.columns(3)
        with c_v1:
            st.write(f"**Nudos (j):** {j_n}")
            st.write(f"**Miembros (m):** {m_e}")
        with c_v2:
            st.write(f"**Reacciones (r):** {r_a}")
            st.write(f"**Fórmula:** $m + r = 2j$")
        with c_v3:
            total_izq = m_e + r_a
            total_der = 2 * j_n
            st.success(f"**{total_izq} = {total_der}**")
            st.info("**Estructura Isostática**")

        st.caption("Esta prueba matemática confirma que la armadura es estable y determinada, cumpliendo con la definición de Estructura Simple.")
# --- Miembros ---
    with tabs[1]:
